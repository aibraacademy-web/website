...
// ─── Helpers pour détecter les valeurs par défaut / placeholders ─────────────

const isCategoryDefault = (cat?: string): boolean => {
  if (!cat) return true;
  ...
  return (
    ...
    normalized === 'non specifie' ||
    normalized === 'général' ||
    normalized === 'general'
  );
};

const isCityDefault = (city?: string): boolean => {
  if (!city) return true;
  const normalized = city.trim().toLowerCase();
  return (
    normalized === '' ||
    normalized === 'autre ville' ||
    normalized === 'autre' ||
...
const isContractTypeDefault = (type?: string): boolean => {
  if (!type) return true;
  ...
  return (
    ...
    normalized === 'non spécifié' ||
    normalized === 'non specifie' ||
    normalized === 'contrat non précisé' ||
    normalized === 'contrat non precise'
  );
};

const isExperienceDefault = (exp?: string): boolean => {
  if (!exp) return true;
  const normalized = exp.trim().toLowerCase();
  return (
    normalized === '' ||
    normalized === 'tous niveaux' ||
    normalized === 'non spécifié' ||
    normalized === 'non specifie'
  );
};

const isSalaryDefault = (sal?: string): boolean => {
  if (!sal) return true;
  const normalized = sal.trim().toLowerCase();
  return (
    normalized === '' ||
    normalized === 'a négocier' ||
...
const isCompanyDefault = (comp?: string): boolean => {
  if (!comp) return true;
  ...
  return (
    ...
    normalized === 'non spécifié' ||
    normalized === 'non specifie' ||
    normalized === 'entreprise confidentielle' ||
    normalized === 'confidentielle' ||
    normalized === 'ec'
  );
};
...
export const JobDetailPage: React.FC<JobDetailPageProps> = ({
  ...
  // Détection des données réelles saisies
  const hasCategory = !isCategoryDefault(job.category);
  const hasCity = !isCityDefault(job.city);
  const hasContractType = !isContractTypeDefault(job.contractType);
  const hasExperience = !isExperienceDefault(job.experienceLevel);
  const hasSalary = !isSalaryDefault(job.salaryRange);

  const hasRealCompany = Boolean(
    (job.company && !isCompanyDefault(job.company)) ||
    job.companyId ||
    job.companyLogo
  );

  // Le bloc "Résumé du poste" n'est affiché que s'il y a au moins un champ structuré pertinent
  const hasStructuredData =
    hasCategory ||
    hasCity ||
    hasExperience ||
    hasSalary ||
    (hasContractType && job.contractType !== 'CDI') ||
    (hasContractType && hasRealCompany);
  ...
  const handleShare = () => {
    const companyPart = hasRealCompany && job.company ? ` chez ${job.company}` : '';
    const cityPart = hasCity && job.city ? ` à ${job.city}` : '';
    const text = encodeURIComponent(`Découvre cette offre d'emploi sur Aibra Academy : ${job.title}${companyPart}${cityPart}.\n${job.contactEmail ? `Contact email: ${job.contactEmail}` : ''}`);
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
              ...
              <div className="flex items-start gap-4">
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white font-extrabold text-2xl flex items-center justify-center shrink-0 shadow-lg border border-emerald-400/30 font-serif overflow-hidden">
                  {job.companyLogo ? (
                    <img src={job.companyLogo} alt={job.company || 'Entreprise'} className="w-full h-full object-contain p-1" />
                  ) : hasRealCompany ? (
                ...
                <div className="space-y-2 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    ...
                    {hasCity && (
                      <span className="bg-white/10 text-slate-200 px-2.5 py-0.5 rounded-full font-medium flex items-center gap-1">
                        <MapPin className="w-3 h-3 text-emerald-400" />
                        {job.city}
                      </span>
                    )}
                    <span className="bg-white/10 text-slate-300 px-2.5 py-0.5 rounded-full font-medium flex items-center gap-1">
                      <Clock className="w-3 h-3 text-emerald-400" />
                      Publié {job.publishedAt || 'récemment'}
                    </span>
                  </div>
          ...
          <div className="p-6 sm:p-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8 text-slate-800">
              ...
              {/* ──────────────────────────────────────────────────────────── */}
              {/* BLOC 1: Résumé du poste (Affiché si données structurées)     */}
              {/* ──────────────────────────────────────────────────────────── */}
              {hasStructuredData && (
                    ...
                    {hasCategory && (
                      <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 flex items-start gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs sm:text-sm font-bold text-slate-800 mt-0.5">{job.category}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {hasCity && (
                      <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 flex items-start gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs sm:text-sm font-bold text-slate-800 mt-0.5">{job.city}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {hasContractType && (
                      <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 flex items-start gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs sm:text-sm font-bold text-slate-800 mt-0.5">{job.contractType}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {hasExperience && (
                      <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 flex items-start gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs sm:text-sm font-bold text-slate-800 mt-0.5">{job.experienceLevel}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {hasSalary && (
                      <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 flex items-start gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-xs sm:text-sm font-bold text-slate-800 mt-0.5">{job.salaryRange}</p>
                        </div>
                      </div>
                    )}
              ...
              {/* ──────────────────────────────────────────────────────────── */}
              {/* BLOC 2: Entreprise (Affiché si entreprise réelle liée)       */}
              {/* ──────────────────────────────────────────────────────────── */}
              {hasRealCompany && (
                <div className="p-6 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-4">
                  <div className="flex items-center gap-2 pb-3 border-b border-slate-100">
                      ...
                      <div>
                        ...
                        {hasCategory && (
                          <p className="text-xs text-slate-500 mt-0.5">
                            Secteur : <span className="font-semibold text-slate-700">{job.category}</span>
                          </p>
                        )}
                      </div>
                    </div>
